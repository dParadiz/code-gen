    ...
    private function getModelData(string $modelName, Schema $schema, Stack $stack): TemplateData\Model
    {
        ...
        if ($schema->type === 'object') {
            ...
            foreach ($schema->allOf as $schemaPartsRef) {
                ...
                } else if ($schemaPartsRef instanceof Schema) {
                    $schemaPart = $schemaPartsRef;
                } else {
                    continue;
                }
                ...
                foreach ($required as $pName => $property) {
                    $this->setModelProperties($data, $pName, $property, $schemaPart, $stack, $modelName);
                }

                foreach ($optional as $allOfPName => $allOfProperty) {
                    $this->setModelProperties($data, $allOfPName, $allOfProperty, $schemaPart, $stack, $modelName);
                }
            }
        }
        return $data;
